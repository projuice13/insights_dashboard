...
export default function NotificationsMenu({
  ...
  const handleApprove = async (n: AppNotification) => {
    setBusyId(n.id);
    try {
      const res = await fetch(`/api/customer-statuses/${encodeURIComponent(n.customerId)}/approve`, { method: 'POST' });
  ...
  const handleReject = async (n: AppNotification) => {
    setBusyId(n.id);
    try {
      const res = await fetch(`/api/customer-statuses/${encodeURIComponent(n.customerId)}/reject`, { method: 'POST' });
      if (!res.ok) throw new Error('Failed');
      setNotifications((prev) => prev.filter((x) => x.id !== n.id));
      router.refresh();
    } catch {
